...
function _buildFilterExprs(filters, expressions, root) {
  var SPECIAL_KEYS = ['$exists', '$contains', '$in', '$lt', '$gt'];
  var BOOLEAN = ['or', 'and'];
  if (!root) {
    root = '';
  }
  ...
  for (var i in filters) {
    if (filters.hasOwnProperty(i)) {
      ...
      if (filters[i].$gt) {
	    ...
	    if (filters[i].$in) {
        var subfilters = filters[i].$in;
        var subexprs = [];
        _buildFilterExprs(subfilters, subexprs, '');
        let subexprsArr = [];
        for (let count = 0; count < subexprs.length; count++) {
          let trimmedSubexp = subexprs[count].split("=");
          subexprsArr.push(trimmedSubexp[1]);
        }
        expressions.push(ident + ' IN ['+subexprsArr+']');
      }
      if (filters[i].$contains) {
        var subfilters = filters[i].$contains;
        var subexprs = [];
        var randomInteger = Math.floor(Math.random() * (9 - 1 + 1)) + 1;
        _buildFilterExprs(subfilters, subexprs, 'x'+randomInteger+'.');
        expressions.push('ANY x'+randomInteger+' IN ' + ident + ' SATISFIES ' +
